#!/bin/bash
# Bring up the v2 stack with Docker Compose and wait for the API to be ready.

set -e

echo "PDF to Speech (v2) - starting up..."
echo
...
if ! grep -q "^GEMINI_API_KEY=AIza" .env 2>/dev/null; then
    echo "Warning: GEMINI_API_KEY not set in .env. v2 needs a Gemini key."
