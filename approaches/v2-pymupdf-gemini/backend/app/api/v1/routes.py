"""API Routes - Gemini-calibrated extraction + Segment-based TTS."""
...
from app.services.pdf.gemini_calibrator import calibrate_with_gemini
...
async def upload_document(file: UploadFile = File(...)):
    ...
    # Step 1: Calibrate with Gemini (sends 3-4 sample pages)
    settings = get_settings()
    profile = await calibrate_with_gemini(
        pdf_path,
        api_key=settings.GEMINI_API_KEY,
        model=settings.GEMINI_MODEL,
    )

    # Step 2: Run calibrated pipeline
    pipeline = PDFProcessingPipeline(profile=profile)
    ...
    _docs[doc_id] = {
        ...
        "segments_ready": {},
        "gemini_profile": result.gemini_profile,
        "toc_source": result.toc_source,
    }

    gemini_used = result.gemini_profile and result.gemini_profile.get("gemini_calibrated", False)
    calibration = "Gemini-calibrated" if gemini_used else "default heuristics"
    ...
    return DocumentUploadResponse(
        ...
        message=f"Processed {result.total_pages} pages, {result.word_count} words. "
                f"Filtered {total_filtered} blocks ({calibration}). "
                f"{len(segments_plan)} audio segments planned.",
    )
...
async def get_gemini_profile(doc_id: str):
    """Debug: see the Gemini layout profile used for this document."""
    doc = _docs.get(doc_id)
    if not doc: raise HTTPException(404, "Not found")
    profile = doc.get("gemini_profile", {})
    return {
        "document_id": doc_id,
        "gemini_calibrated": profile.get("gemini_calibrated", False),
        "toc_source": doc.get("toc_source", "none"),
        "profile": profile,
    }
