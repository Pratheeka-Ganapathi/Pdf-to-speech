...
def split_text_into_segments(text: str) -> list[dict]:
    ...
    Returns segments with correct char_start/char_end in the original text."""
    if not text.strip():
        return []
    ...
    segments = []
    current_text = ""
    current_start = 0   # char offset where current segment starts in original text
    pos = 0             # tracks position in original text
    ...
    for i, para in enumerate(paragraphs):
        ...
        # Would adding this paragraph exceed the segment limit?
        if len(current_text) + len(para) + 2 > SEGMENT_CHARS and current_text:
            # Finalize current segment
            segments.append({
                "text": current_text.strip(),
                "char_start": current_start,
                "char_end": para_start,  # ends where this paragraph starts
            })
            current_text = para
